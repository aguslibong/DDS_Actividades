...
app.get('/api/pacientes', async (req, res) => {
    try {
        const propietario = req.query.propietario; // Usa la P mayúscula si así está en tu base

        let pacientes;

        if (propietario) {
            console.log('Propietario:', propietario); // Verifica el valor
            pacientes = await Paciente.findAll({
                where: {
                    Propietario: propietario, // Match exacto al nombre del campo
                }
            });
        } else {
            pacientes = await Paciente.findAll(); // Trae todos si no se pasó propietario
        }
